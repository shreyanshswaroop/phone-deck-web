...
export default function DeckPage() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-8 overflow-hidden bg-black p-4 text-white">
      <section className="w-[min(86vw,920px)] rounded-[40px] border border-white/10 bg-[#101010] p-8 shadow-[0_0_70px_rgba(255,255,255,0.06)] sm:rounded-[50px] sm:p-10">
        <div className="flex items-start justify-between gap-6">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.5em] text-[#4da3ff] sm:text-base">
              PhoneDeck Cloud
            </p>
            <h1 className="mt-4 text-5xl font-bold tracking-[-0.06em] sm:text-6xl">
              Controller
            </h1>
          </div>

          <div
            className={`flex shrink-0 items-center gap-3 rounded-full px-6 py-3 text-xl font-bold ${
              cloudOnline
        ...
        <div
          className={`mt-8 flex items-center gap-4 text-2xl ${
      ...
      <section
        className="relative aspect-[2.28/1] w-[min(86vw,920px)] touch-pan-y rounded-[58px] bg-[#202020] p-[18px] shadow-[0_0_70px_rgba(255,255,255,0.08)] sm:rounded-[70px] sm:p-[22px]"
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
      >
        <div className="relative h-full w-full overflow-hidden rounded-[42px] bg-black sm:rounded-[56px]">
          <div className="absolute bottom-[13%] left-[3.4%] h-2.5 w-2.5 rounded-full bg-[#22c55e] shadow-[0_0_16px_rgba(34,197,94,0.9)]" />

          <div className="flex h-full items-center justify-center px-[12%] pb-[10%] pt-[8%]">
            {activeSlide === 0 ? (
              <DeckGrid>
                {deckApps.map((app) => (
                  <DeckTile
...
function DeckGrid({ children }: { children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-4 gap-[clamp(18px,3vw,34px)]">
      {children}
    </div>
  );
}
...
function DeckTile({
  ...
  return (
    <button
      ...
      className="group flex h-[clamp(82px,10.5vw,136px)] w-[clamp(82px,10.5vw,136px)] items-center justify-center rounded-[clamp(24px,3vw,38px)] bg-[#171717] shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_18px_24px_rgba(0,0,0,0.45)] ring-1 ring-white/10 transition active:scale-95"
    >
      {image ? (
        <Image
          src={image}
